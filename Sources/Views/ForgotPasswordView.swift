...
struct ForgotPasswordView: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text("新密码（至少 6 位）")
                    .font(.system(size: 13, weight: .medium))
                    .foregroundColor(AppTheme.colors.onMuted)
                AppInput(text: $viewModel.newPassword, placeholder: "请输入新密码")
                    .secure(true)
                    .height(48)
                    .fontSize(16)
                    .cornerRadius(AppTheme.radius.standard)
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text("确认新密码")
                    .font(.system(size: 13, weight: .medium))
                    .foregroundColor(AppTheme.colors.onMuted)
                AppInput(text: $viewModel.confirmPassword, placeholder: "请再次输入新密码")
                    .secure(true)
                    .height(48)
                    .fontSize(16)
                    .cornerRadius(AppTheme.radius.standard)
    ...
    private var resetButton: some View {
        ...
        .height(50)
        .disabled(!viewModel.canReset || viewModel.isLoading)
        .opacity(viewModel.canReset && !viewModel.isLoading ? 1 : 0.6)
    }
    ...
    private func performReset() {
        viewModel.resetPassword { [router] in
            router.navigate(to: .login, style: .root)
        }
    }
}
